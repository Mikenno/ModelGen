...
static const int _MG_OPERATOR_PRECEDENCE_TYPE_COUNT[_MG_OPERATOR_PRECEDENCE_LEVELS] = { 6, 1, 1, 2, 4, 2, 4 };
...
static const MGTokenType _MG_OPERATOR_PRECEDENCE_TYPES[_MG_OPERATOR_PRECEDENCE_LEVELS][_MG_OPERATOR_PRECEDENCE_LONGEST_LEVEL] = {
	...
	{ MG_TOKEN_MUL, MG_TOKEN_DIV, MG_TOKEN_DIVDIV, MG_TOKEN_MOD }
	/* Lowest Precedence */
};
...
static const MGNodeType _MG_ASSIGN_NODE_TYPES[] = {
	MG_NODE_ASSIGN, MG_NODE_ASSIGN_ADD, MG_NODE_ASSIGN_SUB, MG_NODE_ASSIGN_MUL, MG_NODE_ASSIGN_DIV, MG_NODE_BIN_OP_INT_DIV, MG_NODE_ASSIGN_MOD
};

static const MGNodeType _MG_AUG_TO_BIN_OP_TYPES[] = {
	MG_NODE_INVALID, MG_NODE_BIN_OP_ADD, MG_NODE_BIN_OP_SUB, MG_NODE_BIN_OP_MUL, MG_NODE_BIN_OP_DIV, MG_NODE_BIN_OP_INT_DIV, MG_NODE_BIN_OP_MOD
};
...
static MGNode* _mgParseSubexpression(MGParser *parser, MGToken *token, MGbool eatTuple)
{
	...
	else if (token->type == MG_TOKEN_IF)
	{
		...
		while (token->type == MG_TOKEN_ELSE)
		{
			...
			block = mgCreateNode(token, MG_NODE_BLOCK);
			block->token = NULL;
